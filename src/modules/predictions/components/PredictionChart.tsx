...
const PredictionChart: React.FC<PredictionChartProps> = ({ data }) => {
  ...
  return (
    <Card className="rounded-2xl bg-card border-border/10 text-white w-full flex flex-col min-h-[450px]">
      <CardHeader>
        <CardTitle className="text-xl font-bold">Prediction Distribution</CardTitle>
      ...
      <CardContent className="flex-1 flex flex-col justify-end pb-8 pt-4 px-6 relative">
        {/* Chart Area */}
        <div className="relative w-full flex-1 min-h-[250px]" style={{ minHeight: "250px" }}>
          ...
          <div className="absolute inset-0 w-full h-full flex justify-around items-end pt-4 pb-0 z-10 pl-2">
            {data.map((item, index) => (
              <div key={index} className="flex flex-col items-center justify-end h-full w-1/3 group relative">
      ...
      <div className="p-4 border-t border-white/5 flex justify-center gap-8 bg-black/20">
         <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-primary-green">Home / Yes</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-primary-red rounded-sm" />
            <span className="text-xs font-medium text-primary-red">Away / No</span>
          </div>
      </div>
    </Card>
  );
